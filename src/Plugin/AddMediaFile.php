 ...
 * 
 * Replaces AddMedia plugin with AddMediaFile plugin that 
 * looks for media source field names in a more robust way,
 * including field_media_file and checks the md5sum.
 * 
 */
 ...
 */
class AddMediaFile extends AbstractIbPlugin
{
    /**
     * Constructor.
     *
                ...
                foreach ($media['field_media_use'] as $term_index => $term) {
                    if (count($this->settings['drupal_media_tags']) == 0 ||
                        ...
                        if ($term_index == 0) {
                            ...
                            // check sha256 hash if configured to do so
                            if ($this->settings['check_media_file_hashes'] === true) {
                                $calculated_hash = hash_file('sha256', $temp_file_path);
                                $expected_hash = $file_json['sha256'][0]['value'];
                                if ($calculated_hash !== $expected_hash) {
                                    $this->logger->error(
                                        "Hash mismatch for media file. Expected: [$expected_hash], Calculated: [$calculated_hash]",
                                        array(
                                            'file_url' => $file_url,
                                            'drupal_url' => $this->settings['drupal_base_url'],
                                        )
